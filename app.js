...
function renderImport(d){d=d.filter(x=>!SaeSource.cancelled(x));currentImport=d;renderKPIs("import",d);renderPending("import",d);renderDonut("import",d)}function renderMfg(d){renderKPIs("mfg",d);renderPending("mfg",d);renderManufactureSummary(d);renderManufactureDonut(d)}
...
function renderDonut(p,d){if(typeof Chart==="undefined")return;const c={};d.forEach(x=>{const s=has(x["Overall status"])?txt(x["Overall status"]):"No Status";c[s]=(c[s]||0)+1});const canvas=document.getElementById(p==="import"?"importStatusChart":"mfgStatusChart"),old=p==="import"?importChart:mfgChart;if(old)old.destroy();canvas.setAttribute("role","img");canvas.setAttribute("aria-label",Object.entries(c).map(([label,count])=>label+": "+count).join("; ")||"No data available");const chart=new Chart(canvas,{type:"doughnut",plugins:[statusDepth("TNO",d.length)],data:{labels:Object.keys(c),datasets:[{data:Object.values(c),backgroundColor:["#1769e0","#26a34a","#f58213","#ef2b1f","#eeb308","#94a3b8"],borderWidth:2,borderColor:"#fff"}]},options:{responsive:true,maintainAspectRatio:false,cutout:"64%",layout:{padding:{top:3,bottom:12}},plugins:{legend:{position:"right",labels:{boxWidth:10,font:{size:10}}},tooltip:{enabled:true}}}});if(p==="import")importChart=chart;else mfgChart=chart}
