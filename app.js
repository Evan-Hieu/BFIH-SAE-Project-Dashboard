let IMPORT=[],MFG=[],importChart=null,mfgChart=null,currentImport=[];document.addEventListener("DOMContentLoaded",init);
function init(){
  ...
  document.getElementById('importPendingBody').innerHTML='<tr><td colspan="14" class="empty">Click Google Sheet to load SAE data</td></tr>';
...
function renderPending(p,d){const rows=d.filter(x=>!SaeSource.dispatched(x)&&!SaeSource.cancelled(x)).sort((a,b)=>(b._gap??-999999)-(a._gap??-999999));set(p==="import"?"importPendingCount":"mfgPendingCount",rows.length+" items");const body=document.getElementById(p==="import"?"importPendingBody":"mfgPendingBody");body.innerHTML=rows.length?rows.map(x=>p==="import"?importPendingRow(x):`<tr><td><span class="priority ${x._priority.cls}">${x._priority.text}</span></td><td>${esc(x["Project"])}<br><b>${esc(x["Build"])}</b></td><td>${esc(x["Machine/Equipment name"])}</td><td>${esc(date(x["NBD"]))}</td><td>${esc(x["BFIH site arrive"])}</td><td class="${gapcls(x._gap)}">${x._gap===null?"":x._gap>0?"+"+x._gap:x._gap}</td><td class="stage">${esc(x._stage)}</td><td>${esc(x._action)}</td></tr>`).join(""):`<tr><td colspan="${p==="import"?14:8}" class="empty">No data available</td></tr>`}
...
function lang(l){langEn.classList.toggle("active",l==="en");langZh.classList.toggle("active",l==="zh");exportButton.textContent=l==="zh"?"导出":"Export";importButton.textContent=l==="zh"?"导入":"Import"}
...
function importPendingRow(x){
  return `<tr><td><span class="priority ${x._priority.cls}">${esc(x._priority.text)}</span></td>
    ...
    <td>${esc(x['KO QTY'])}</td>
    <td>${esc(x['Dispatched Qty'])}</td>
    <td>${esc(x['Pending qty'])}</td>
