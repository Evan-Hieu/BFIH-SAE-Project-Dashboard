(() => {
  ...
  const editable=()=>!snapshot?.readOnly&&!!window.WebAuth?.enabled&&WebAuth.can(active==='import'?'Import':'Manufacture',true)&&SheetConnection.access(configs[active].id).canEdit;
  ...
  async function readSource(config){
    if(SheetConnection.sharedMode?.()&&!SheetConnection.identity()){
      const data=await SheetConnection.readShared(config.id);
      return {raw:data.values,values:data.values,readOnly:true};
    }
