...
assert.equal(mapped[0].text,'08/17');
assert.equal(mapped[23].text,'08/17');
...
context.window.InhouseTimeline.setSyncError('Missing ACT ETD');
assert(nodes.timelineMessage.textContent.includes('Missing ACT ETD'));
assert(nodes.timelineMatrixBody.innerHTML.includes('could not be loaded'));
assert(!nodes.timelineMatrixBody.innerHTML.includes('No matching equipment'));
context.window.InhouseTimeline.update([]);
assert(!nodes.timelineMessage.textContent.includes('sync failed'));
assert(nodes.timelineMatrixBody.innerHTML.includes('No matching equipment'));
console.log('PASS 24 timeline source coordinates, status references, dates, red quantities and rendering.');

assert.equal(item._timelineRemark, String(row[16]));
context.window.InhouseTimeline.update([{Project:'Q test',Equipment:'Fixture',Condition:'Do not show condition',Remark:'Do not show R',_timelineRemark:'Status from Q'}]);
assert(nodes.timelineMatrixBody.innerHTML.includes('Status from Q'));
assert(!nodes.timelineMatrixBody.innerHTML.includes('Do not show'));
